...
const signup = async (req, res) => {   
    ...
    try {  
        // Username validation  
        const existingUser = await Dealer.findOne({ username });  
        if (existingUser) {  
...
const login = async(req,res)=>{
    ...
    try{  
        const userdata= await Dealer.findOne({username})
         if(!userdata)
         {
...
const getMe = async (req, res) => {  
    try {  
        const user = await Dealer.findById(req.user._id);  
...
const forget = async (req, res) => {  
    try {  
        ...
        const user = await Dealer.findOne({ username, email });  
        if (!user) {  
